...
module.exports = async (event) => {
	...
	return books.length > 1 ? getMultipleBooks(event) : getSingleBook(event, books);
};
...
async function getMultipleBooks(event) {
	...
	let text = `Of the genres below, click your ${choicesLeft === 1 ? 'top' : choicesLeft} favorite${choicesLeft > 1
		? 's'
		: ''}!`;
